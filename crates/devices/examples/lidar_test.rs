        ...
        if driver.is_frame_ready() {
            if let Some(cloud) = driver.get_point_cloud() {
                ...
                if let Some(stats) = calculate_statistics(&cloud.points) {
                    println!("  Distance range: {} - {} mm", stats.min_dist, stats.max_dist);
                ...
                if !front_points.is_empty() {
                    let closest_front = front_points.iter()
                        .filter(|p| p.is_valid())
                        .min_by_key(|p| p.distance);
                    
                    if let Some(point) = closest_front {
                        println!("  Closest obstacle ahead: {} mm at {:.1}°", 
...
fn calculate_statistics(points: &[Point]) -> Option<Statistics> {
    ...
    let min_dist = valid_points.iter()
        .map(|p| p.distance)
        .min()
        .unwrap();
    
    let max_dist = valid_points.iter()
        .map(|p| p.distance)
        .max()
        .unwrap();
    
    let sum_dist: u32 = valid_points.iter()
        .map(|p| p.distance as u32)
        .sum();
    let avg_dist = sum_dist as f32 / valid_points.len() as f32;
    
    let sum_intensity: u32 = valid_points.iter()
        .map(|p| p.intensity as u32)
        .sum();
